import React from 'react'
import logo from '../assets/searchmagnifier_logo.svg'
// import logo from '../assets/youtube.png'
function Header() {
  return (
    <div className='flex'>
      <div className='flex'>
      <img src="https://cdn.iconscout.com/icon/free/png-256/free-hamburger-menu-462145.png?f=webp" alt="hamburger_menu" className='h-8 w-9' />
      <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTjdB5idttrtpfZkhv3Dz20jvA14SXgOCOKQiBT6_tnEyagCBi91sEJMTU8X6eLxzb5dQ&usqp=CAU" alt="youtube_logo" className='h-6 mt-1' />
    </div>
    <div className='pl-4'>
    <input type="text" placeholder='search' className='rounded-left'/>
    <button><img src={logo} alt="magnify_icon" className='w-8 h-6 pt-2' /></button>
    </div>
    <div>
    <img src="data:image/png;base64," alt="user_profile" className='h-9' />  
    </div> 
    </div>
  )
}

export default Header;